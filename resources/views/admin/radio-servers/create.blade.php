...
@section('title', 'Add Radio Server')

@section('content')
<div class="admin-header">
    <div>
        <h1 class="admin-title">Add Radio Server</h1>
        <p class="admin-subtitle">Configure a new Icecast or Shoutcast server</p>
    </div>
...
<form action="{{ route('admin.radio-servers.store') }}" method="POST">
    @csrf
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                    ...
                    <option value="icecast" {{ old('type') === 'icecast' ? 'selected' : '' }}>Icecast</option>
                    <option value="shoutcast" {{ old('type') === 'shoutcast' ? 'selected' : '' }}>Shoutcast</option>
                </select>
                @error('type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <div class="col-md-8">
                    <div class="form-group">
                        ...
                        <input type="text" name="host" id="host" class="form-control @error('host') is-invalid @enderror" 
                               value="{{ old('host', 'localhost') }}" placeholder="localhost or IP address" required>
                        @error('host')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" name="port" id="port" class="form-control @error('port') is-invalid @enderror" 
                               value="{{ old('port', 8000) }}" min="1" max="65535" required>
                        @error('port')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-check">
                <input type="checkbox" name="ssl" id="ssl" class="form-check-input" value="1" {{ old('ssl') ? 'checked' : '' }}>
    ...
    <div class="card mt-3">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" name="admin_user" id="admin_user" class="form-control" value="{{ old('admin_user') }}">
            </div>

            <div class="form-group">
                <label for="admin_password">Admin Password</label>
                <input type="password" name="admin_password" id="admin_password" class="form-control" value="{{ old('admin_password') }}">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" name="docker_host" id="docker_host" class="form-control" 
                       value="{{ old('docker_host', 'local') }}" placeholder="local or remote Docker host URL">
            ...
            <div class="form-group">
                ...
                <input type="text" name="docker_container_name" id="docker_container_name" class="form-control" 
                       value="{{ old('docker_container_name') }}" placeholder="my-radio-server">
            </div>

            <div class="form-group">
                <label for="docker_image">Docker Image</label>
                <input type="text" name="docker_image" id="docker_image" class="form-control" 
                       value="{{ old('docker_image') }}" placeholder="moul/icecast:latest or mbentley/shoutcast:latest">
                <small class="form-text text-muted">Leave blank to use default image for server type</small>
            </div>

            <div class="form-check">
                <input type="checkbox" name="auto_start" id="auto_start" class="form-check-input" value="1" {{ old('auto_start') ? 'checked' : '' }}>
...
@vite('resources/js/modules/admin-radio-server-create.js')
