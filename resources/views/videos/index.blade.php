    ...
    <div class="card">
        ...
        <div class="card-body">
            <p style="color: var(--color-text-secondary); margin-bottom: 1.5rem;">
                Watch YLYL videos, streamer clips, and more from the Los Santos Radio community.
            </p>

            {{-- Category Navigation --}}
            <div style="display: flex; gap: 1rem; margin-bottom: 2rem; flex-wrap: wrap;">
                <a href="{{ route('videos.ylyl') }}" class="btn btn-primary" style="display: flex; align-items: center; gap: 0.5rem;">
                    <i class="fas fa-laugh-squint"></i>
                    YLYL Videos
                </a>
                <a href="{{ route('videos.clips') }}" class="btn btn-primary" style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    @foreach($videos as $video)
                        <a href="{{ route('videos.show', $video) }}" class="video-card card" style="text-decoration: none; color: inherit;">
                            <div style="position: relative; height: 180px; background: var(--color-bg-tertiary);">
                                ...
                                @if($video->category)
                                    <div style="position: absolute; top: 0.5rem; left: 0.5rem; background: var(--color-accent); color: white; padding: 0.25rem 0.5rem; border-radius: 0.25rem; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">
                                        {{ $video->category }}
                                    </div>
                                @endif
                            </div>
            ...
            @else
                <div style="text-align: center; padding: 3rem;">
                    <i class="fas fa-video" style="font-size: 4rem; color: var(--color-text-muted); margin-bottom: 1rem; display: block;"></i>
                    <h3 style="color: var(--color-text-secondary); margin-bottom: 0.5rem;">No Videos Yet</h3>
                    <p style="color: var(--color-text-muted);">Check back later for awesome videos!</p>
                </div>
            @endif
        </div>
    </div>
